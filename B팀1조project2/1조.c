...
int exercise_time;
int exercise_satisfaction;

int main(void) {
	int valid_input = 0;

	printf("오늘의 운동 시간을 기록해주세요. 예) 1시간 23분 -> 0123 : ");
	while (!(valid_input = scanf("%d", &exercise_time))) {
		printf("네 자리 정수 형태로 입력해주세요. : ");
		while (getchar() != '\n') continue;
	}

	valid_input = 0;

	printf("오늘 추천 받은 운동을 만족하시나요? (1.네 / 2.아니요)\n");
	while (!valid_input) {
		scanf_s("%d", &exercise_satisfaction);
		if (exercise_satisfaction == 1) {
			printf("감사합니다. \n");
			valid_input = 1;
		}
		else if (exercise_satisfaction == 2) {
			printf("다시 운동을 추천드리겠습니다.\n");
			printf("\n*******************\n");
			printf("운동의 불만족 요인을 선택하시면 알맞는 운동 루틴을 추천드리겠습니다.\n");

			int choice;   // 선택값
			int choice1, choice2, choice3, choice4; // 운동	불만족 요인에 따른 운동 종류 선택값

			// 운동 불만족 요인 목록 출력
			printf("\n불만족 요인를 선택하세요.\n");
			printf("1. 심폐지구력 향상 부족\n2. 유연성 향상 부족\n3. 근육량 증가 부족\n4. 체지방 감소 부족\n");
			printf("\n운동 불만족 요인: ");
			// 선택값 입력
			...
			{
			case 1: // 심폐 지구력 향상 부족
				printf("\n[심폐 지구력 향상 부족]을 불만족 요인으로 설정하셨습니다.\n");
